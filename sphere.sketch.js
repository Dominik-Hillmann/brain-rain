...
// essential variables for constructing of the sphere
var sphereRadius;                                    // how radius of circle representing sphere
var cursor;
var startPoint;
var middlePoint;
var layers = [];
...
function setup()
{
   ...
   else
   {
      ...
      /*
      for(var i = 1; i <= COLOR_GRADIENTS.length; i++)
      {
         if (r < (i / COLOR_GRADIENTS.length))
            cg = COLOR_GRADIENTS[i];
      }
      */
...
function draw()
{
   ...
   for(var i = 0; i < layers.length; i++)
   {
      ...
      layers[i].drawPoints(2, 6, width);
   }
}

// This is a setup function that is called if the browser window being resized.
// It is slightly different from setup, e.g. it keeps the color gradient that is set in setup() and does not choose a new one.
function setupOnResize()
{
   ...
   canvas = createCanvas(width, height);        // reset old canvas
   canvas.position(0, 0);
